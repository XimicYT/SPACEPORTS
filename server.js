...
const FRICTION = 0.98; // Increased slightly so they roll smoother
...
io.on("connection", (socket) => {
  socket.on("initSession", (sessionId) => {
    ...
    socket.emit("gameState", { players, doors: activeDoors, balls });
  });
...
setInterval(() => {
  ...
  for (const sessionId in players) {
    if (now - players[sessionId].lastHeartbeat > 15000) removePlayer(sessionId);
  }
}, 5000);
...
setInterval(() => {
  ...
  if (closedCount < MAX_CLOSED_DOORS && Math.random() < 0.6) {
    ...
    if (openIndices.length > 0) {
      ...
      doors[pick].cooldownUntil = doors[pick].closeUntil + 1500;
    }
  }
}, 1000);
...
setInterval(() => {
  ...
  balls.forEach((ball) => {
    ...
    for (const pid in players) {
      ...
      if (dist < minDist && dist > 0) {
        ...
        // Separate completely to prevent sticky hits
        ball.x += nx * overlap;
        ball.y += ny * overlap;

        // Unify units: Client sends units/sec, ball operates in units/frame
        const pVxFrame = p.vx / 60;
        const pVyFrame = p.vy / 60;

        // Relative Velocity (Are they moving towards each other?)
        const relV = (pVxFrame - ball.vx) * nx + (pVyFrame - ball.vy) * ny;

        if (relV > 0) {
          // Elastic Collision Impulse! Ball gains momentum, player handled on client
          ball.vx += nx * relV * 1.5;
          ball.vy += ny * relV * 1.5;
        }
      }
    ...
    balls.forEach((otherBall) => {
      ...
      if (dist < minDist && dist > 0) {
        ...
        const relV =
          (ball.vx - otherBall.vx) * nx + (ball.vy - otherBall.vy) * ny;
        if (relV > 0) {
          ball.vx -= nx * relV * 0.8;
          ball.vy -= ny * relV * 0.8;
          otherBall.vx += nx * relV * 0.8;
          otherBall.vy += ny * relV * 0.8;
        }
      }
    });

    // SAFETY: Hard cap the ball's speed so it can never tunnel
    ball.vx = Math.max(-60, Math.min(60, ball.vx));
    ball.vy = Math.max(-60, Math.min(60, ball.vy));
  ...
  // CRITICAL FIX: Use volatile emits! Drops queued packets to ensure the client is ALWAYS receiving real-time data.
  io.volatile.emit("gameState", { players, doors: activeDoors, balls });
}, 1000 / 60);
